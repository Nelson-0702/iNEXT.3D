...
# CoverageFD -------------------------------------------------------------------
CoverageFD = function(data, datatype, m){
      ...
      if(nboot>1){
        ...
        ses <- sapply(1:nboot, function(B){
          ...
          qFDm_b <- FD.m.est(ai_vi = Boot_aivi,m = m[[i]],q = q,nT = n) %>%
            t() %>% as.numeric()
          covm_b = CoverageFD(Boot.X[,B], datatype, m[[i]])
      ...
      if(nboot>1){
        ...
        ses <- sapply(1:nboot, function(B){
          ...
          qFDm_b <- FD.m.est(ai_vi = Boot_aivi,m = m[[i]],q = q,nT = nT) %>%
            t() %>% as.numeric()
          covm_b = CoverageFD(Boot.X, datatype, m[[i]])
          return(c(qFDm_b,covm_b))
        }) %>% apply(., 1, sd)
      }else{
        ses <- rep(NA,length(c(qFDm,covm)))
...
invChatFD_abu <- function(ai_vi, data_, q, Cs, tau){
  n <- sum(data_)
  refC = CoverageFD(data_, 'abundance', n)
  f <- function(m, cvrg) abs(CoverageFD(data_, 'abundance', m) - cvrg)
...
invChatFD_inc <- function(ai_vi, data_, q, Cs, tau){
  n <- data_[1]
  refC = CoverageFD(data_, 'incidence_freq', n)
  f <- function(m, cvrg) abs(CoverageFD(data_, 'incidence_freq', m) - cvrg)
  mm <- sapply(Cs, function(cvrg){
    if (refC > cvrg) {
      opt <- optimize(f, cvrg = cvrg, lower = 0, upper = n)
      mm <- opt$minimum
