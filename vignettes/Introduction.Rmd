...
## DATA INFORMATION FUNCTION: DataInfo()
...
AO3D(
  ...
  nboot = 50,conf = 0.95,nT = NULL,Method,
...
```{r}
out1 <- AO3D(dunes$data, diversity = 'TD', datatype = "abundance", method = c('Estimated', 'Empirical'),nboot = 5,conf = 0.95)
...
tree <- data.inc$tree
nT <- data.inc$nT
out2 <- AO3D(data, diversity = 'PD', q = c(0, 1, 2), datatype = "incidence_raw", nT = nT, nboot = 5, method = c('Estimated', 'Empirical'), PDtree = tree, PDreftime = seq(0.1, 82.8575, length.out = 40))
...
data <- dunes$data
distM <-  dunes$dist
out3 <- AO3D(data, diversity = 'FD', q = c(0, 1, 2), datatype = "abundance", nboot = 5, method = c('Estimated', 'Empirical'), FDtau = seq(0, 1, 0.1), FDdistM = distM, FDtype = 'tau_values')
